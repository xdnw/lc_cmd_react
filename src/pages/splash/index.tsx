...
export function setupAnimation(): AnimationControls {
  ...
  const w = canvas.width = horrorCanvas.width = document.documentElement.clientWidth;
  const h = canvas.height = horrorCanvas.height = document.documentElement.clientHeight;
  const cx = w / 2;
  const cy = h / 2;
  ...
  const rotCos = Math.cos(config.rotationSpeed);
  const rotSin = Math.sin(config.rotationSpeed);
  ...
  class StarObj {
    ...
    draw(ctxOff: CanvasRenderingContext2D): void {
      ...
      if (isWarping) {
        this.z += config.warpSpeed * (1 + warpFactor * 2.8);
        this.x *= 0.92; this.y *= 0.92;
        if (this.z > config.depth) this.resetDist();
      } else {
        const nx = this.x * rotCos - this.y * rotSin;
        const ny = this.x * rotSin + this.y * rotCos;
        this.x = nx; this.y = ny;
  ...
  const stars: StarObj[] = Array.from({ length: config.starCount }, () => new StarObj());
  ...
  function drawPostWarpHorror() {
    ...
    hctx.globalAlpha = 1;
    const tearBands = 8 + Math.floor(intensity * 10);
  ...
  function updateVoidState() {
    ...
    } else if (voidTarget > 0 && --voidHoldFrames <= 0) {
      voidTarget = 0;
    }

    const easing = voidTarget > voidStrength ? 0.07 : 0.03;
    voidStrength += (voidTarget - voidStrength) * easing;
    voidPhase += 0.01;
  ...
  function tickEye() {
    ...
    if (eyePhase === 0) {
      if (--eyeTimer <= 0 && !isWarping) eyePhase = 1;
    } else if (eyePhase === 1) {
      eyeOpen = Math.min(1, eyeOpen + 0.005);
      if (eyeOpen >= 1) { eyePhase = 2; eyeHoldTimer = 180; }
    } else if (eyePhase === 2) {
      if (--eyeHoldTimer <= 0) eyePhase = 3;
    } else {
      eyeOpen = Math.max(0, eyeOpen - 0.004);
  ...
  function animate() {
    ...
    if (isWarping) {
      warpFrame = Math.min(warpFrame + 1, warpFrames);
    ...
    if (postWarpIntensity > 0) {
      postWarpFrame = Math.min(postWarpFrame + 1, postWarpFrames);
    ...
    if (!glitchActive) {
      ...
      if (--glitchHoldFrames <= 0) glitchActive = false;
    }

    if (!isWarping && Math.random() < 0.0012) dreadPulse = 1;
    dreadPulse *= 0.93;
    updateVoidState();
    ...
    for (let i = 0; i < stars.length; i++) stars[i].draw(offscreenCtx);
    ...
    if (isWarping) {
      ...
      for (let i = 0; i < 28; i++) {
        const angle = (i / 28) * Math.PI * 2 + time * 0.008;
